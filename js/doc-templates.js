...
// ── Landing Page Live Template Visualizer Controller ──────────────────
window.KivoVisualizer = {
  ...
  specs: {
    minimalist: {
      name: 'Minimalist',
      badge: 'Épuré & Essentiel',
      desc: 'Clair et concis, design épuré officiel KIVO. Idéal pour les consultants, freelances et prestataires de services qui privilégient la sobriété, l\'élégance discrète et la lisibilité maximale.'
    },
    corporate: {
      name: 'Corporate',
      badge: 'Recommandé B2B',
      desc: 'Formel et structuré avec un bandeau bleu marine contrasté. Le standard par excellence pour les entreprises, PME, cabinets et échanges inter-entreprises.'
    },
    modern: {
      name: 'Modern',
      badge: 'Agences & Créatifs',
      desc: 'Vibrant et distinctif avec un subtil dégradé violet/rose et des badges arrondis. Conçu spécialement pour les studios créatifs, agences digitales et startups.'
    },
    elegant: {
      name: 'Elegant',
      badge: 'Luxe & Prestige',
      desc: 'Raffiné avec typographie noble à empattements et liserés dorés. Parfait pour les maisons de prestige, l\'événementiel haut de gamme, l\'artisanat d\'art et les architectes.'
    },
    clean: {
      name: 'Modern Clean',
      badge: 'Précis & Chirurgical',
      desc: 'Clair et géométrique avec un liseré cyan moderne. Excellente lisibilité des lignes de facturation et mise en valeur instantanée des conditions de paiement.'
    },
    editorial: {
      name: 'Editorial',
      badge: 'Impact Typographique',
      desc: 'Typographie percutante et fort contraste noir et blanc rehaussé d\'une touche rouge vif. Idéal pour l\'édition, la production média et la communication visuelle.'
    },
    premium: {
      name: 'Premium Dark',
      badge: 'Édition Sombre Exclusive',
      desc: 'Édition exclusive sur fond sombre texturé avec des finitions cuivre brossé et or. Offre une signature visuelle inoubliable pour les prestations et offres VIP.'
    }
  },

  getSampleData: function () {
    const isQuote = this.docType === 'quote';
    const num = isQuote ? 'DEV-2026-0042' : 'FAC-2026-0042';
    const subtotal = 450000;
    const tax = 81000;
    const grandTotal = 531000;

    return {
      biz: {
        name: 'KIVO MATIQUE SARL',
        address: '12 Avenue Hassan II, Dakar',
        phone: '+221 33 821 00 00',
        email: 'contact@kivo.sn',
        logoText: 'KIVO',
      ...
      dueDate: '20/09/2026',
      status: this.isPaid ? 'paid' : 'sent',
      paymentMethod: 'Wave / Orange Money / Carte Bancaire',
      terms: 'Paiement à 15 jours par Wave, CB ou virement',
      notes: isQuote 
        ? 'Ce devis est valable 30 jours. Merci de nous le retourner signé avec mention bon pour accord.' 
        : 'Facture acquittée avec KIVO MATIQUE. Merci pour votre collaboration.',
      client: {
        name: 'Studio Pulse SARL',
        company: 'Studio Pulse SARL',
        phone: '+221 77 123 45 67',
        email: 'compta@studiopulse.sn',
        address: 'Route des Almadies, Dakar',
        taxId: 'SN-DKR-2026-B-9988'
      },
      items: [
        { name: 'Conception UI/UX & Design System complet', quantity: 1, price: 250000, taxRate: 18, total: 250000 },
        { name: 'Intégration Passerelle Wave & Carte Bancaire', quantity: 1, price: 150000, taxRate: 18, total: 150000 },
        { name: 'Hébergement Cloud Sécurisé & Maintenance annuelle', quantity: 1, price: 50000, taxRate: 18, total: 50000 }
      ],
      subtotal: subtotal,
      discount: 0,
      taxAmount: tax,
  ...
  select: function (templateId) {
    if (!this.specs[templateId]) return;
  ...
  renderVisualizer: function () {
    ...
    // Update active nav pills
    document.querySelectorAll('.kivo-vis-tab').forEach(btn => {
      const tmpl = btn.getAttribute('data-template');
      btn.classList.toggle('active', tmpl === this.activeTemplate);
    });

    // Update active cards in bottom gallery
    document.querySelectorAll('.kivo-vis-card').forEach(card => {
      const tmpl = card.getAttribute('data-template');
      card.classList.toggle('active-vis-card', tmpl === this.activeTemplate);
    });

    // Update segmented buttons
    const btnInvoice = document.getElementById('vis-btn-invoice');
    const btnQuote = document.getElementById('vis-btn-quote');
    if (btnInvoice && btnQuote) {
      btnInvoice.classList.toggle('active', this.docType === 'invoice');
      btnQuote.classList.toggle('active', this.docType === 'quote');
    }

    const btnPaid = document.getElementById('vis-btn-paid');
    const btnPending = document.getElementById('vis-btn-pending');
    if (btnPaid && btnPending) {
      btnPaid.classList.toggle('active', this.isPaid === true);
      btnPending.classList.toggle('active', this.isPaid === false);
    }

    // Update details texts
    const spec = this.specs[this.activeTemplate] || this.specs.corporate;
    const titleEl = document.getElementById('vis-title');
    const badgeEl = document.getElementById('vis-badge');
    const descEl = document.getElementById('vis-desc');
    const filenameEl = document.getElementById('vis-doc-name');

    if (titleEl) titleEl.textContent = spec.name;
    if (badgeEl) badgeEl.textContent = spec.badge;
    if (descEl) descEl.textContent = spec.desc;
    if (filenameEl) {
      const prefix = (this.docType === 'quote') ? 'DEV-2026-0042' : 'FAC-2026-0042';
      filenameEl.textContent = `${prefix}_${this.activeTemplate}.pdf`;
    }

    // Smooth render of the real document
    canvas.style.opacity = '0.35';
    canvas.style.transform = 'scale(0.99)';
    setTimeout(() => {
      const sample = this.getSampleData();
      canvas.innerHTML = window.KivoTemplates.render(this.activeTemplate, sample);
  ...
  init: function () {
    const gallery = document.getElementById('landing-vis-gallery');
    if (gallery && window.KivoTemplates) {
      gallery.innerHTML = window.KivoTemplates.builtIn.map(t => {
        const spec = this.specs[t.id] || { badge: 'Modèle Pro' };
        const isActive = t.id === this.activeTemplate;
        return `
          <div class="kivo-vis-card ${isActive ? 'active-vis-card' : ''}" data-template="${t.id}" onclick="KivoVisualizer.select('${t.id}')">
            <div class="kivo-vis-card-thumb">
              ${window.KivoTemplates.miniPreview(t.id)}
            </div>
            <div class="kivo-vis-card-body">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.35rem;">
                <h4 class="kivo-vis-card-title">${t.name}</h4>
                <span style="font-size: 0.65rem; font-weight: 700; color: #2563EB; background: #EFF6FF; padding: 2px 7px; border-radius: 9999px;">${spec.badge}</span>
              </div>
              <p class="kivo-vis-card-desc">${t.desc}</p>
              <button class="kivo-vis-card-btn" type="button">
                <span>Afficher dans le visualiseur</span>
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => window.KivoVisualizer && window.KivoVisualizer.init());
} else {
  setTimeout(() => window.KivoVisualizer && window.KivoVisualizer.init(), 50);
}

console.log('[KivoTemplates] ✅', window.KivoTemplates.builtIn.length, 'templates disponibles. Visualiseur initialisé.');
